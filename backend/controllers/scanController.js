...
const pdfScan = async (req, res) => {
  try {
    ...
    if (ef.length > 0) {
      has = true;
      addReco(
        "Restrict Information Disclosure & Block Sensitive Files Access",
        `Critical development or configuration files (${ef.join(', ')}) are publicly accessible. Update your web server configuration (.htaccess, nginx.conf) to restrict access to these resources immediately.`
      );
    }
